...
const SplashButton = ({ children, className, ...rest }: ButtonProps) => {
  return (
    <button
      className={twMerge(
        "bg-gradient-to-br from-blue-400 to-blue-700 px-4 py-2 rounded-md ring-2 ring-blue-500/50 hover:ring-transparent active:ring-blue-500/70 ring-offset-2 ring-offset-zinc-950 text-lg text-zinc-50 transition-all hover:scale-[1.02] active:scale-[0.98]",
        className
      )}
      {...rest}
    >
...
const BubbleButton = ({ children, className, ...rest }: ButtonProps) => {
  return (
    <button
      className={twMerge(
        `relative z-0 before:-z-10 before:absolute before:inset-0 flex items-center before:content-[""] gap-2 border-zinc-700 before:bg-zinc-100 bg-gradient-to-br from-zinc-800 to-zinc-950 px-3 py-1.5 border rounded-md before:rounded-[100%] text-zinc-50 hover:text-zinc-900 whitespace-nowrap transition-all before:transition-transform before:translate-y-[200%] duration-300 overflow-hidden before:scale-[2.5] before:duration-500 hover:scale-105 hover:before:translate-y-[0%] active:scale-100`,
        className
      )}
      {...rest}
    >
...
const CornerGrid = () => {
  return (
    <div
      ...
      className="top-0 right-0 z-0 absolute size-[50vw]"
    >
      <div
        style={{
          backgroundImage:
